(function (global, factory) {
	...
	var scene = (html => {
	  ...
	  const onlyImage = hasImage ? hasText.innerHTML === hasImage.outerHTML : false; // heading scene
	...
	var blocks = ['image', 'youtube', 'svg', 'video', 'vegalite', 'chartjs', 'fitty'];
	var block$2 = (src => {
	...
	var main = (str => {
	  ...
	  scenes.forEach(d => {
	    if (d !== '\n---\n') {
	      ...
	      const blk = block$2(srcHtml); // create the scene object

	      const scn = scene(blk.src); // add additional blocks from definition, if any

	      scn.blocks = scn.blocks.concat(blk.blocks); // check if there's only one block

	      const oneBlock = scn.blocks.length === 1 ? scn.blocks[0] : null; // parse directives and set into the scene

	      const dir = directives(text);
	      dir.forEach(d => {
	        for (const k in d) {
	          scn[k] = d[k]; // only if there's one block, we propagate the properties also to the block level
